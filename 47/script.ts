...
const showTestimonial = (showing: number) => {
  ...
  text.textContent = testimonials[showing].text;
  name.textContent = testimonials[showing].name;
  position.textContent = testimonials[showing].position;
  photo.src = testimonials[showing].photo;
};

showTestimonial(showing);

setInterval(() => {
  showing++;
  if (showing >= testimonials.length) {
    showing = 0;
  }
  showTestimonial(showing);
}, 10000);
